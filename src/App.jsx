...
function App() {
            ...
            userArray.forEach((userObject) => {
                ...
                if (userObject.tasks && Array.isArray(userObject.tasks)) {
                    userObject.tasks.forEach((taskObject) => {
                        ...
                        const task = new Task(`${taskDescription} for ${userName} at ${taskTime}`, null, taskTime);
    ...
    const handleSubmit = () => {
        const employees = employeeInput.split('\n').map((line) => {
            const [name, jobTitle, shiftStart, shiftEnd] = line.split(',').map((value) => value.trim());
